...
barrier \emph{shape} is essentially settled: a real-$\sqrt{\,}$ quadratic layer
$D^2/(K\sqrt r)$ with a large fixed $K$ is, numerically, a supersolution for $c=137$,
with margin growing like $\sqrt r$. Exactly one task remains: a rigorous derivation
and proof of the (SS) inequality for this $\varphi$ at a fixed large $K$ - the
binding region is the upper-middle of the strip, not the diagonal, so the diagonal
ODE above is only motivational and the earlier scalar reduction $Q(D)$ must be
redone. No square-root discretization issue remains. This single construction is the
only ungreen node between Proposition~\ref{prop:comparison} and
Theorem~\ref{thm:question}.
