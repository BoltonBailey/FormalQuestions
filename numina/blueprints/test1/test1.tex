...
\paragraph{The discretization obstruction.}
This continuum picture does not transfer verbatim, because the theorem's region uses
$\lfloor\sqrt r\rfloor$ and $\varphi$ must be $\mathbb Q$-valued. Replacing $\sqrt r$
by $m=\lfloor\sqrt r\rfloor$ makes $\varphi$ rational but injects $O(1)$ jumps in $m$
at perfect squares and distorts the strip width by $O(1)$. Numerically the floor
barrier then fails near perfect-square neighbours (e.g. $r=9999$ at $K=91$) and at
small $r$, and - unlike the continuum case - increasing $K$ does \emph{not}
uniformly help (it worsens small-$r$ points): no single $K$ tested passes for all
$r>137$ (e.g. $K=500$ fails near $r=537$ and $r=10299$). A rigorous $\mathbb
Q$-valued barrier therefore needs a monotone rational surrogate for $\sqrt r$ (so the
$r$-difference keeps a controlled sign), or an enlarged constant plus an auxiliary
correction absorbing the $O(1)$ floor errors.
...
\paragraph{Status.}
The barrier \emph{shape} is essentially settled: a quadratic layer
$D^2/(K\sqrt r)$ with a large fixed $K$ is, numerically, a supersolution in the
continuum for $c=137$, with margin growing like $\sqrt r$. Two things remain for a
rigorous proof: (a) a correct derivation and proof of the continuum (SS) inequality
for a fixed large $K$ - the binding region is the upper-middle of the strip, not the
diagonal, so the diagonal ODE above is only motivational and the earlier scalar
reduction $Q(D)$ must be redone; and (b) a $\mathbb Q$-valued discretization
controlling the $\lfloor\sqrt r\rfloor$ jumps and the small-$r$ regime. This single
construction is the only ungreen node between Proposition~\ref{prop:comparison} and
